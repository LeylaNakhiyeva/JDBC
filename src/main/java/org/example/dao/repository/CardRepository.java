...
public class CardRepository {
    public void saveCard(Card card){
        try (Connection connection = DatabaseConnection.getDatabaseConnection()){
            PreparedStatement preparedStatement = connection.prepareStatement("insert into card(customer_id, card_number, cvv, currency, created_at, is_active) values(?, ?, ?, ?, ?, ?)");
    ...
    private Card buildCardFromResultSet(ResultSet rs) throws SQLException {
        ...
        String cvv = rs.getString("patronymic");
        CurrencyEnum currency = (CurrencyEnum) rs.getObject("currency");    // ???
        LocalDate createdAt = rs.getDate("created_at").toLocalDate();
        LocalDate updateAt = rs.getDate("update_at")== null ? null : rs.getDate("update_at").toLocalDate();
        Boolean isActive = rs.getBoolean("is_active");

        return  new Card(id, customerId, card_number, cvv, currency, createdAt, updateAt, isActive);
    }
}
